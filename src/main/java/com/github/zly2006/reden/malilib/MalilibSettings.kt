...
@JvmField val UNDO_KEY = ConfigHotkey("撤回", "LEFT_CONTROL,Z", "").generic().hotkey()
@JvmField val REDO_KEY = ConfigHotkey("撤销", "LEFT_CONTROL,Y", "").generic().hotkey()
@JvmField val UNDO_SUPPORT_LITEMATICA_OPERATION = ConfigBoolean("关闭撤回", true, "停用撤回功能").generic()
@JvmField val UNDO_CHEATING_ONLY = ConfigBoolean("关闭撤销", true, "仅在启用作弊功能时才启用撤销功能.").generic()
...
@JvmField val RVC_RECORD_MULTIPLAYER = ConfigBoolean("记录多人游戏", true, "启用多人游戏中的 RVC").rvc()
@JvmField val RVC_FORCE_LOCALLY = ConfigBoolean("rvcForceLocally", false, "即使远程服务器启用了 RVC，也要强制 RVC 在本地运行").rvc()
@JvmField val CHAT_RIGHT_CLICK_MENU = ConfigBoolean("聊天启用菜单", true, "在聊天中启用启用菜单").sr()
@JvmField val DEBUG_LOGGER = ConfigBoolean("调试程序", false, "启用调试记录器").debug()
