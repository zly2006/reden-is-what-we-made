...
@JvmField
val NO_TIME_OUT = RedenConfigBoolean("noTimeOut").generic()
...
@JvmField
val SOLID_FACE_RENDERER = RedenConfigBoolean("solidFaceRenderer").generic()
@JvmField
val ENABLE_CLIENT_GLOW = RCBooleanHotkey("enableClientGlow", true, "LEFT_CONTROL,G").hotkey().generic()
...
// Micro Tick
@JvmField
val BREAKPOINT_RENDERER = RCBooleanHotkey(
    "breakpointRenderer", false, "B",
    KeybindSettings.INGAME_BOTH
).hotkey().mt()
...
@JvmField
val TOGGLE_FORCE_ENTITY_POS_SYNC =
    RCBooleanHotkey("toggleForceEntityPosSync", false, "LEFT_CONTROL,LEFT_SHIFT,V").hotkey().mt()
// RVC
...
// Debug
@JvmField
val DEBUG_LOGGER = RedenConfigBoolean("debugLogger") {
    if (booleanValue) startDebugAppender()
    else stopDebugAppender()
}.debug()
@JvmField
val DEBUG_PACKET_LOGGER = RedenConfigBoolean("debugPacketLogger").debug()
@JvmField val DEBUG_TAG_BLOCK_POS = RedenConfigHotkey("debugTagBlockPos", "LEFT_CONTROL,LEFT_SHIFT,T").debug().hotkey()
@JvmField
val DEBUG_LOGGER_IGNORE_UNDO_ID_0 = RedenConfigBoolean("debugLoggerIgnoreUndoId0").debug()
...
@JvmField
val DO_ASSERTION_CHECKS = RedenConfigBoolean("doAssertionChecks").debug()
@JvmField
val UNDO_REPORT_UN_TRACKED_TNT = RedenConfigBoolean("undoReportUnTrackedTnt").debug()
...
@JvmField
val DEVELOPER_MODE = RedenConfigBoolean("developerMode").debug()
@JvmField val LOCAL_API_BASEURL = RedenConfigString("localApiAddr", "http://localhost:10005/api").debug()
@JvmField
val ENTITY_OUTLINE_RENDER_RAW = RedenConfigBoolean("entityOutlinrRenderRaw").debug()
@JvmField
val DEBUG_DISPLAY_RVC_WORLD_INFO = RedenConfigHotkey("debugDisplayRvcWorldInfo").debug().hotkey()
@JvmField
val DEBUG_NEW_NOTIFICATION = RedenConfigHotkey("debugNewNotification", "").debug().hotkey()
@JvmField
val DEBUG_LITEMATICA_SCHEMATIC_RERENDER = RedenConfigHotkey("debugLitematicaSchematicRerender").debug().hotkey()
...
object HiddenOption {
    ...
    @JvmField
    val iPRIVACY_SETTING_SHOWN = RedenConfigBoolean("iPrivacySettingShown").hidden()
